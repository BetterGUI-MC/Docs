...
* You can use [`/getvariables`](basic/command.md) to get the list of registered variables
...
* There are variables are generated by the menu.
  * You can use [`/getvariables <menu>`](basic/command.md) to get the list of registered variables for the specific menu
* To use variables from another menu: `{menu_<menu_name>.yml_<variable>`
  * Example: If you use [Store Argument Processor](argument/store.md) in a menu called `test.yml` and want to use `{merged_args}` in other menus, you can use `{menu_test.yml_merged_args}`
